...
export function generateAndSaveKeypair (paths: Paths): KeypairRecord {
...
export function loadKeypair (paths: Paths): Keypair {
  if (!fs.existsSync(paths.keyFile)) {
    throw new Error(`No institution keypair found at ${paths.keyFile}. Generate one first.`)
  }
...
export function loadPublicKeyHex (paths: Paths): string {
  const record: KeypairRecord = JSON.parse(fs.readFileSync(paths.keyFile, 'utf8'))
